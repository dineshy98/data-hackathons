...
sec_lev_data = pd.concat([data1.loc[data1['risk_flag'] == 0].sample(10000),data1.loc[data1['risk_flag'] == 1].sample(10000)])

sec_lev_data['risk_flag'].value_counts()


data1.drop(sec_lev_data.index,inplace = True)
data1.reset_index(drop = True,inplace = True)


data0_cat = data1[data1['risk_flag'] == 0].reset_index(drop = True)
data1_cat = data1[data1['risk_flag'] == 1].reset_index(drop = True)
...
lgbm.fit(X_tr_cat1, y_tr_cat1,eval_set=(X_tst_cat1,y_tst_cat1))
...
for i in range(4):
    ...
    globals()['xgb%s' % i] = XGBClassifier(n_estimators= 1000)
    globals()['xgb%s' % i].fit(globals()['X_tr_cat%s' % i], globals()['y_tr_cat%s' % i])
    score['xgb{}'.format(i)] = roc_auc_score(globals()['y_tst_cat%s' % i],globals()['xgb%s' % i].predict(globals()['X_tst_cat%s' % i]))
...
for i in range(4):
    ...
    preds['xgb{}'.format(i)] = globals()['xgb%s' % i].predict_proba(sec_lev_data.drop(columns = ['risk_flag']))[:,1]
